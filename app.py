...
# Config
# --------------------------------------------------------------------------- #
VERSION = "2.2.0"   # bump this on every deploy so you can confirm the new code is running
...
def poll_once():
    ...
    with _lock:
        for did, r in results.items():
            ...
            # detect status change for flash signal
            prev = _prev_status.get(did)
            r["status_changed"] = (prev is not None and prev != r["status"])
            _prev_status[did] = r["status"]

            # inject derived fields
            ut = _uptime[did]
            r["uptime_pct"] = round(100 * ut["online"] / ut["polls"]) if ut["polls"] else None
            ls = _last_seen.get(did)
            r["last_seen_ago"] = round(now - ls) if ls else None
            r["history"] = list(_history[did])

    families = _aggregate(results)
    dur = time.time() - t0
    with _lock:
        _state["devices"]  = results
  ...
  <div class="modal">
    ...
    <div class="row2">
      <div class="field"><label>IP Address</label>
        <input id="f-ip" placeholder="10.0.0.x"/></div>
      <div class="field"><label>Hostname</label>
        <input id="f-hostname" placeholder="device.extron.video"/></div>
    </div>
